...
fs.readdir(videoDir, async (err, files) => {
  ...
  for (let i = 0; i <= files.length - 1; i++) {
    const file = files[i];
    // if (file.isDirectory()) {
    //   fs.readdir(videoDir + '/' + file, async (err, childFiles) => {
    //     if (err) {
    //       error(err)
    //       return;
    //     }
    //     for (let j = 0; j <= childFiles.length - 1; j++) {
    //       const current = childFiles[j];
    //       await dealFile(current, file)
    //     }
    //   })
    // } else {
    //   await dealFile(file)
...
async function dealFile(file, pathdir) {
  if (file.endsWith('.mp4')) {
    log('开始处理文件：', file);
    try {
      const fileName = file.split('.')[0];
      const wavFile = pathdir ? `${videoDir}/${pathdir}/${fileName}.wav` : `${videoDir}/${fileName}.wav`;
      const srtFile = `${renderFilePath(sourceSrtSaveName, fileName)}`;
      await extractAudio(pathdir ? `${videoDir}/${pathdir}/${file}` : `${videoDir}/${file}`, `${wavFile}`);
      ...
      if (translateServiceProvider) {
        await app(pathdir ? videoDir + '/' + pathdir : videoDir, fileName, `${srtFile}.srt`);
      }
      log('翻译完成');
